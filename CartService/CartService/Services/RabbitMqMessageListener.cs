...
) : IMessageListener<T>
{
    ...
    private async Task InitializeListener()
    {
        try
        {
            ...
            await channel.QueueDeclareAsync(queue: options.Value.CartProductMessagesQueue,
                durable: true,
                exclusive: false,
                autoDelete: false,
                arguments: null);
            ...
            await channel.QueueBindAsync(options.Value.CartProductMessagesQueue,
            ...
            await channel.BasicConsumeAsync(queue: options.Value.CartProductMessagesQueue,
                autoAck: true,
                consumer: consumer);
